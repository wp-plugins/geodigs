		...
		<div class="form-group col-md-6">
			...
			<select class="form-control" name="cities">
				...
				<?foreach($cities as $city):?>
					<option value="<?=$city->name?>" <? selected($city->name, gd_get_search_param('cities')); ?>><?=$city->name?></option>
				<?endforeach;?>
			</select>
			<label>County</label>
		...
		<div class="row">
			<div class="col-md-4">
				...
				<select class="form-control" name="minBed">
					<option value="">Any</option>
					<? for ($i = 1; $i < 13; $i++): ?>
						<option value="<?=$i?>" <? if ($i == gd_get_search_param('minBed')) echo 'selected'; ?>><?=$i?>+</option>
					<? endfor; ?>
				</select>
				...
				<select class="form-control" name="minBath">
					<option value="">Any</option>
					<? for ($i = 1; $i < 13; $i++): ?>
						<option value="<?=$i?>" <? if ($i == gd_get_search_param('minBath')) echo 'selected'; ?>><?=$i?>+</option>
					<? endfor; ?>
				</select>
...
<!-- 					<label>Master Bedroom Level</label>
					<select name="masterBedLevel">
						...
						<?foreach($listing_levels as $level):?>
							<option value="<?=$level->id?>"  <? selected($level->id, gd_get_search_param('masterBedLevel')); ?>><?=$level->readable;?></option>
						<?endforeach;?>
					</select> -->
				<?php endif; ?>
			</div>
...
<!-- 							<label>Finished Basement</label>
					<select class="form-control" name="finishedBasement">
						<option value="">Any</option>
						<option value="1" <? selected(gd_get_search_param('finishedBasement'), 1) ?>>Yes</option>
						<option value="0" <? selected(gd_get_search_param('finishedBasement'), 0) ?>>No</option>
					</select> -->
				<?php endif; ?>
			</div>
			<div class="col-md-4">
				...
				<div class="row">
					<div class="col-xs-6">
						<select class="form-control" name="minAboveArea">
							...
							<?for($i = 1; $i < 15; $i++):
								$value = $i * 500; ?>
								<option value="<?=$value?>" <? selected($value, gd_get_search_param('minAboveArea')); ?>>At least <?=number_format($value) . "sqft";?></option>
							<?endfor;?>
						</select>
					</div>
					<div class="col-xs-6">
						<select class="form-control" name="maxAboveArea">
							...
							<?for($i = 1; $i < 15; $i++):
								$value = $i * 500; ?>
								<option value="<?=$value?>" <? selected($value, gd_get_search_param('maxAboveArea')); ?>>At most <?=number_format($value) . "sqft";?></option>
							<?endfor;?>
						</select>
					</div>
				</div>
				...
				<div class="row">
					<div class="col-xs-6">
						<select class="form-control" name="minFinishedArea">
							...
							<?for($i = 1; $i < 15; $i++):
								$value = $i * 500; ?>
								<option value="<?=$value?>" <? selected($value, gd_get_search_param('minFinishedArea')); ?>>At least <?=number_format($value) . "sqft";?></option>
							<?endfor;?>
						</select>
					</div>
					<div class="col-xs-6">
						<select class="form-control" name="maxFinishedArea">
							...
							<?for($i = 1; $i < 15; $i++):
								$value = $i * 500; ?>
								<option value="<?=$value?>" <? selected($value, gd_get_search_param('maxFinishedArea')); ?>>At most <?=number_format($value) . "sqft";?></option>
							<?endfor;?>
						</select>
					</div>
				</div>
				...
				<div class="row">
					<div class="col-xs-6">
						<select class="form-control" name="minTotalArea">
							...
							<?for($i = 1; $i < 15; $i++):
								$value = $i * 500; ?>
								<option value="<?=$value?>" <? selected($value, gd_get_search_param('minTotalArea')); ?>>At least <?=number_format($value) . "sqft";?></option>
							<?endfor;?>
						</select>
					</div>
					<div class="col-xs-6">
						<select class="form-control" name="maxTotalArea">
							...
							<?for($i = 1; $i < 15; $i++):
								$value = $i * 500; ?>
								<option value="<?=$value?>" <? selected($value, gd_get_search_param('maxTotalArea')); ?>>At most <?=number_format($value) . "sqft";?></option>
							<?endfor;?>
						</select>
					</div>
				</div>
				...
				<select class="form-control" name="minParkingSpaces">
					<option value="">Any</option>
					<?for($i = 1; $i < 6; $i++): ?>
						<option value="<?=$i?>" <? selected($i, gd_get_search_param('minParkingSpaces')); ?>>At least <?= ($i == 1) ? "1 space" : "$i spaces";?></option>
					<?endfor;?>
				</select>
	...
	<? if ($listing_alert_form): ?>
		...
		<div id="gd-listing-alerts-settings" class="form-group row">
			<div class="col-sm-4">
				...
				<div class="checkbox">
					<label>
						<input type="checkbox" name="email" <? checked($search->email, '1') ?> value="1">
						Alert via email
					</label>
				</div>
				<!-- Text -->
<!-- 				<div class="checkbox">
					<label>
						<input type="checkbox" name="text" <? checked($search->text, '1') ?> value="1">
						Alert via text
					</label>
				</div> -->
				<!-- Frequency -->
				...
				<select name="freq" required>
					<option value="daily" <? selected('daily', $search->freq); ?>>Daily</option>
					<option value="instant" <? selected('instant', $search->freq); ?>>Instant</option>
					<option value="never" <? selected('never', $search->freq); ?>>Never</option>
				</select>
			</div>
		</div>
	<? endif; ?>
